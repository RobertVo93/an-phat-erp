...
interface ICustomerDetailClientProps {
  id: string
  initialData: ICustomerDetailPageData
}

type CustomerOrderRow = Order & { id: string }

export function CustomerDetailClient({ id, initialData }: ICustomerDetailClientProps) {
  const { t } = useLanguage()
  const {
    loading,
    notFoundError,
    ...
    handlePageChange,
    handleSort,
  } = useCustomer(id, initialData)
  const getStatusText = useCallback((status: string) => t(`orders.status.${status}`), [t])

  const tableRows = orders.filter((order): order is CustomerOrderRow => Boolean(order.id))

  const columns: ServersideTableColumn<CustomerOrderRow>[] = [
    {
      key: "number",
    ...
    },
    {
      ...
      render: (row) => (
        <Badge className={getOrderStatusColor(row.status!)}>{row.status ? getStatusText(row.status) : "-"}</Badge>
      ),
    },
    {
      key: "paymentStatus",
  ...
  if (notFoundError || !customer) return null
