				...
				<div key={item.id} className="p-3 border rounded-lg space-y-3">
					<div className="flex items-start justify-between">
						<div className="flex-1 min-w-0">
							...
							<p className="text-xs text-gray-500">SKU: {item.number}</p>
						</div>
					...
					<div className="grid grid-cols-3 gap-3">
						<div className="space-y-1">
							...
							<div className="flex items-center space-x-1">
								<Button type="button" variant="outline" size="sm" onClick={() => updateQuantity(index, item.quantity! - 1, getWarehouseProductTotal(item.id!))} className="w-8 p-0">
									<Minus className="h-3 w-3" />
								</Button>
								<Input
									type="number"
									value={item.quantity}
									onChange={e => updateQuantity(index, Number.parseInt(e.target.value) || 1, getWarehouseProductTotal(item.id!))}
									className="w-20 text-center text-xs"
									min="1"
									max={getWarehouseProductTotal(item.id!)}
								/>
								<Button type="button" variant="outline" size="sm" onClick={() => updateQuantity(index, item.quantity! + 1, getWarehouseProductTotal(item.id!))} className="w-8 p-0">
