...
export async function getOrders(params: OrderFilters = {}) {
  const url = new URL(withApiBase("/api/orders"), window.location.origin);
...
export async function getOrderById(id: string) {
  const res = await fetch(withApiBase(`/api/orders/${id}`));
  return res.json();
}

export async function createOrder(data: Partial<Order>) {
  const res = await fetch(withApiBase("/api/orders"), {
    method: "POST",
