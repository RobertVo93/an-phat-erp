...
export async function getCollections(params: CollectionFilters = {}) {
  const url = new URL(withApiBase("/api/collections"), window.location.origin);
...
export async function getCollectionById(id: string) {
  const res = await fetch(withApiBase(`/api/collections/${id}`), { credentials: "include" });
  if (!res.ok) throw new Error("Failed to fetch collection");
  return res.json();
}

export async function createCollection(data: Partial<Collection>) {
  const res = await fetch(withApiBase("/api/collections"), {
    method: "POST",
...
export async function updateCollection(id: string, data: Partial<Collection>) {
  const res = await fetch(withApiBase(`/api/collections/${id}`), {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify(data),
